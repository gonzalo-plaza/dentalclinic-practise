...
function updateAppoinmentsTable(){
    ...
    for(const date in appointments){
        ...
        <td>${appointmentHour}</td><td>${appointments[date].name}</td><td>${appointments[date].surname}</td><td class="test-touch">${appointments[date].phone}</td><td><i class="test-desk appointment__option fa-sharp fa-solid fa-phone"></i><i class="appointment__option fa-solid fa-trash js-delet-appointment" data-bs-toggle="modal" data-bs-target="#exampleModal" title="Eliminar cita"></i><i class="appointment__option fa-solid fa-pen-to-square js-edit-appointment" title="Ver y editar cita"></i></td>`;
